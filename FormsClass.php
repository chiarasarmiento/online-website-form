...
class FormsClass {
	...
	function time($name='',$class='',$id='',$attrib='') {
		...
		$input = '<div class="input-group clockpicker" data-align="left" data-donetext="Done">
				<input type="text"  name="'.$fldname.'" class="form-control '.$class.'" value="'.$value.'" id="'.$id.'" '.$attrib.'>
...
//radio
	...
	function radio($name='',$value='',$id='',$attrib='',$rows=''){
		...
		foreach($value as $radVal){
			...
			$radio .= '<td'.$style.'><input type="radio" name="'.$fldname.'" value="'.$radVal.'" '.$cndtn.' id="'.$fldname.$count.'" '.$attrib.'>'.'<label for="'.$fldname.$count.'"style="font-weight:normal;">'.$radVal.'</label></td>'."\n";
			$brekz++;
			$count++;
		}
		$radio .= "</tr></table>";
	...
	function amount($name='',$id='',$attrib='') {
		...
		$input = '<div class="field_holder animated_box"> <i id="icon" class="fas fa-dollar-sign"></i> <input type="text" name="'.$fldname.'" class="form_field amount" id="currency-field" pattern="^\\$?(([1-9](\\d*|\\d{0,2}(,\\d{3})*))|0)(\\.\\d{1,2})?$" value="" data-type="currency" placeholder="Enter amount here"><span class = "animated_class" ></span></div>';
