...
function SignaturePad (selector, options, nameData) {
  /**
  ...
  function validateForm () {
    var valid = true
      ...
      , name_Field = $(settings.output).attr('name')

    // nameData === name attr of input
    if(nameData === "xsx"){
      valid = true
    }else{
...
$.fn.signaturePad = function (options) {
  ...
  this.each(function () {
    if (!$.data(this, 'plugin-signaturePad')) {
      var nameData = $(this).find('input').attr('name');
        api = new SignaturePad(this, options , nameData)
        api.init()
